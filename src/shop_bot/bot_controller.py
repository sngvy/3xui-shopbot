"""Управление жизненным циклом основного Telegram-бота (запуск/остановка)."""
...
class BotController:
    """Управляет жизненным циклом основного Telegram-бота: запуск, остановка, статус."""

    def __init__(self):
        """Инициализирует контроллер в состоянии "бот не запущен"."""
        self._dp = None
        self._bot = None
        self._task = None
    ...
    def set_loop(self, loop: asyncio.AbstractEventLoop):
        """Сохраняет ссылку на event loop, в котором будет работать бот."""
        self._loop = loop
        logger.info("Цикл событий установлен.")

    def get_bot_instance(self) -> Bot | None:
        """Возвращает текущий экземпляр Bot или None, если бот не запущен."""
        return self._bot
    ...
    async def _start_polling(self):
        ...
        try:
            ...
            logger.error(
                f"Критическая ошибка при запуске опроса: {e}",
                exc_info=True,
            )
        finally:
    ...
    def start(self):
        """Запускает бота в уже установленном event loop. Возвращает словарь со статусом."""
        if self._is_running:
        ...
        try:
            ...
            if not isinstance(user_router, Router):
                raise TypeError(
                    f"get_user_router() must return Router instance, got: {type(user_router)}"
                )
            if not isinstance(admin_router, Router):
                raise TypeError(
                    f"get_admin_router() must return Router instance, got: {type(admin_router)}"
                )
            ...
            yoomoney_enabled = (str(ym_flag).lower() in ("true", "1", "yes", "on")) and bool(
                ym_wallet
            )
    ...
    def stop(self):
        """Останавливает работающего бота. Возвращает словарь со статусом."""
        if not self._is_running:
    ...
    async def run_auto_start(self):
        """Прямой запуск бота без возврата словарей."""
        logger.info("Попытка автозапуска бота...")
        result = self.start()
        if result["status"] == "error":
            logger.error(f"Автозапуск не удался: {result['message']}")
        else:
            logger.info("Автозапуск: команда успешно передана в цикл событий.")
