// Единые цвета сетки и подписей для всех графиков панели.
// Значения берутся из токенов темы, поэтому мониторинг и главная
// выглядят одинаково и корректно переключаются между темами.
(function () {
    if (typeof Chart === 'undefined') return;
    try {
        var isDark = document.documentElement.getAttribute('data-bs-theme') === 'dark';
        // Фолбэк на случай, если CSS-переменная почему-то ещё не читается
        // (например, сработал раньше применения стилей). Раньше здесь был
        // всегда светлый вариант, из-за чего сетка терялась именно в
        // тёмной теме — а она в проекте стоит по умолчанию.
        var fallbackGrid = isDark ? 'rgba(255,255,255,.12)' : 'rgba(0,0,0,.10)';
        var fallbackTick = isDark ? '#9b93a8' : '#666666';
        var cs = getComputedStyle(document.documentElement);
        var grid = (cs.getPropertyValue('--m3-chart-grid') || '').trim();
        var tick = (cs.getPropertyValue('--m3-chart-tick') || '').trim();
        Chart.defaults.borderColor = grid || fallbackGrid;
        Chart.defaults.color = tick || fallbackTick;
        // Доступно графикам, которые (как на Главной) не задают grid.color
        // явно на каждой шкале, а также тем, что создаются позже/динамически.
        window.m3ChartGrid = function () { return grid || fallbackGrid; };
        window.m3ChartTick = function () { return tick || fallbackTick; };
    } catch (e) {}
})();

document.addEventListener('DOMContentLoaded', function () {
    // Переносим все модалки прямо в <body>, минуя любых промежуточных
    // предков (main, .container и т.д.). Бэкдроп Bootstrap всегда
    // добавляет как последнего ребёнка <body> — если модалка лежит
    // ГДЕ-ТО ГЛУБЖЕ во вложенности, а не прямо в <body>, сравнение
    // z-index между ней и бэкдропом может считаться в разных контекстах
    // наложения, и бэкдроп визуально/по кликам оказывается "выше" модалки
    // независимо от выставленных z-index. Перенос в body устраняет
    // саму возможность такой ситуации, а не борется с её симптомами.
    ...
    // Сервер отдаёт HTML с отступами и переносами, а node.innerHTML браузер
    // нормализует. Прямое сравнение почти всегда давало «не равно», поэтому
    // блок перерисовывался и мигал на каждом цикле, даже когда данные те же.
    ...
    window.refreshContainerById = async function(id){
        ...
        try {
            ...
            if (html && !sameHtml(html, node.innerHTML)) {
                ...
                node.innerHTML = html;
                try {
                    if (!node.hasAttribute('data-fetch-quiet')) {
    ...
    function initializePasswordToggles() {
        ...
        togglePasswordButtons.forEach(button => {
            ...
            const setIcon = () => {
                ...
                // Material Symbols: меняем лигатуру внутри .msym.
                // Если иконки нет — создаём её (раньше здесь были эмодзи).
                let icon = button.querySelector('.msym');
                if (!icon && !button.querySelector('svg')) {
                    icon = document.createElement('span');
                    icon.className = 'msym sm';
                    button.textContent = '';
    ...
    // Как и в dashboard.html: сюда нельзя передавать undefined "на всякий
    // случай" — Chart.js затирает им унаследованный дефолт вместо отката.
    function m3Grid(){
        if (window.m3ChartGrid) return window.m3ChartGrid();
        return document.documentElement.getAttribute('data-bs-theme') === 'dark' ? 'rgba(255,255,255,.12)' : 'rgba(0,0,0,.10)';
    }
    function m3Tick(){
        if (window.m3ChartTick) return window.m3ChartTick();
        return document.documentElement.getAttribute('data-bs-theme') === 'dark' ? '#9b93a8' : '#666666';
    }
    function initializeDashboardCharts() {
        ...
        function updateChartFontsAndLabels(chart) {
            ...
            // Раньше подписи осей и сетка полностью пропадали на экранах
            // <=470px, а затем и на <=360px (это очень частая ширина
            // Android-телефонов в портретной ориентации — 360×640/740/800),
            // из-за чего при повороте экрана или скрытии адресной строки
            // подписи то появлялись, то исчезали. Теперь они остаются на
            // любой ширине, просто становятся мельче и реже.
            const isVerySmall = window.innerWidth <= 360;
            chart.options.scales.x.ticks.font.size = (window.innerWidth <= 470) ? (isVerySmall ? 8 : 9) : (isMobile ? 10 : 12);
            chart.options.scales.y.ticks.font.size = (window.innerWidth <= 470) ? (isVerySmall ? 8 : 9) : (isMobile ? 10 : 12);
            chart.options.plugins.legend.labels.font.size = isMobile ? 12 : 14;
            chart.options.scales.x.ticks.maxTicksLimit = (window.innerWidth <= 470) ? (isVerySmall ? 3 : 4) : (isMobile ? 8 : 15);
    ...
    function initializeTicketAutoRefresh() {
        ...
        function updateStatus(status) {
            if (status === 'open') {
                statusEl.innerHTML = '<span class="badge bg-green">Открыт</span>';
                ...
                statusEl.innerHTML = '<span class="badge">Закрыт</span>';
                const textarea = document.getElementById('reply-text');
                const replyBtn = document.getElementById('reply-btn');
    ...
    function initializeSoftAutoUpdate() {
        ...
        nodes.forEach(node => {
            ...
            tick();
            timer = setInterval(tick, Math.max(60000, interval));
